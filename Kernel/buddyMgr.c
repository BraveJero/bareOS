...
static uint8_t * mem_base = TOTAL_MEMORY - TOTAL_HEAP_SIZE;
...
void free(void * address) {
	...
	do {
		...
		uint8_t index = INDEX_OF_POINTER_IN_LEVEL((uint64_t)address, level, (uint64_t)mem_base, HEAP_SIZE);
		uint64_t buddy;
...
void mem_dump() {
      ...
      for (int i = 0; i < levels; i++) {
            ...
            if (!list_is_empty(list)) {
                  ...
                  ncNewline();
                  int sum = 0;
                  for (following = list->next, index = 0; following != NULL; index++, following = following->next) {
                        sum++;
                  }
...
static int getFirstAvailableLevel(int minLevel) {
    ...
    for (selectedLevel = minLevel; list_is_empty(&allLists[selectedLevel]); selectedLevel--);
